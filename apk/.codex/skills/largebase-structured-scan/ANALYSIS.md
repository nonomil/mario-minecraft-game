...
| 无机器可读格式 | 只有 Markdown | Codex 需手动解析，易出错 |
| 无索引/搜索 | 线性查找 | 查找信息低效 |
...
4. **Phase D：质量检查（Codex）**
   - 验证输出完整性
   - 检查关键信息

5. **Phase E：路由决策（Codex）**
   - 根据执行摘要路由
...
## 与 codex-workflow-largebase.md 的衔接
...
python3 .codex/skills/largebase-structured-scan/scan.py \
  --mode M1 \
  --scope src/ \
  --topic "feature-x"
