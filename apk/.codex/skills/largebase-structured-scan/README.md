...
flowchart TD
    ...
    subgraph PhaseA["Phase A：预检（Codex 执行）"]
    ...
    subgraph PhaseD["Phase D：查询验证 + 路由"]
        ...
        D3 -->|=1| R1[→ codex-workflow-complex.md]
        D3 -->|≥2| R2[→ codex-workflow-parallel.md]
        D3 -->|缺陷主导| R3[→ codex-workflow-debug.md]
    end
```
...
```bash
# Step 1：初始化扫描目录
python .codex/skills/largebase-structured-scan/scan.py scan \
  --mode M4 \
  --scope .codex .claude docs \
...
# Step 3：写入 SQLite
python .codex/skills/largebase-structured-scan/scan.py load \
  --load docs/scan/YYYY-MM-DD-codebase-full-scan/scan-data.json \
  --db   docs/scan/YYYY-MM-DD-codebase-full-scan/scan.db

# Step 4：验证查询
python .codex/skills/largebase-structured-scan/scan.py query \
  --query hybrid_search \
  --type all \
  --db docs/scan/YYYY-MM-DD-codebase-full-scan/scan.db

# Step 5（可选）：摘要写入 CLAUDE.md
python .codex/skills/largebase-structured-scan/scan.py export-to-claude-md \
  --db docs/scan/YYYY-MM-DD-codebase-full-scan/scan.db \
  --claude-md CLAUDE.md
```
...
| 问题 | 状态 | 说明 |
|------|------|------|
| Cartographer 未安装 | ✅ 已修复 | git clone 到 `compat plugin directory` |
| tiktoken 缺失 | ✅ 已修复 | `pip install tiktoken` |
| pwsh 不可用 | ⚠️ 待修复 | `run_largebase_scan.ps1` 无法运行，需 Python 替代脚本 |
| bash hook 路径错误 | ⚠️ 待修复 | `compat plugin directory` 下子 git 仓库导致 hook 路径解析失败 |
