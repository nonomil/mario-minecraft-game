(() => {
  ...
  const EXTENSION_CANDIDATE_WORDS = [
    ...
    "方向盘", "候车亭", "停车位", "警示牌", "指示牌", "安全线", "安全门", "消防车", "救护车", "警报器",
  ...
  const LOW_GRADE_CLASSROOM_WORDS = [
    ...
    "队礼", "队歌", "广播操", "课后", "下课", "上课", "放学", "班会", "课代表", "班干部",
    "黑板擦", "白板擦", "粉笔盒", "图书柜", "读书台", "展示角", "评比栏", "书角", "提醒卡", "任务单",
    "作业本", "语文书", "数学书", "音乐书", "美术书", "科学书", "活动课", "阅读课", "写字台", "讲台桌",
    "座位表", "值日卡", "读书台", "课桌椅", "收作业", "发本子", "交作业", "整理柜", "校门队", "队列表"
  ];
  ...
  const LOW_GRADE_COMMUNITY_WORDS = [
    ...
    "天桥", "路灯", "路口", "过马路", "警察叔叔", "消防员", "救护车", "值班室", "保健室", "医务角",
    "图书馆", "博物馆", "文化馆", "少年宫", "活动站", "服务台", "售票处", "停车位", "候车亭", "公交卡",
    "门铃声", "电梯口", "楼梯间", "安全员", "提醒牌", "提示牌", "告示栏", "规则牌", "节日卡", "春游",
    "秋游", "参观", "集合点", "解散点"
  ];
  ...
  const SECOND_GRADE_SCENE_WORDS = [
    ...
    "种植角", "观察瓶", "观察盒", "昆虫盒", "采集盒", "借书袋", "阅读单", "朗读单", "练习单", "实验单",
    "识字条", "拼音条", "句式卡", "好词卡", "好句卡", "写话卡", "说话卡", "摘抄本", "摘抄卡", "读书单",
    "轮流表", "分组表", "值日册", "早读表", "生字条", "词语表", "图书漂流", "阅读任务", "课桌角", "图书袋"
  ];

  const SECOND_GRADE_READING_WORDS = [
    "整句", "整段", "分段", "分句", "段意", "题目", "题意", "图意", "文意", "读题",
    "审题", "圈词", "找词", "抄词", "摘词", "摘句", "短句", "长句", "连句", "写段",
    "词句卡", "句子条", "段落卡", "自然段", "写话纸", "读书条", "朗读条", "朗读卡", "复述条", "复述卡",
    "观察卡", "记录卡", "识字表", "识字本", "生字条", "词语条", "句式表", "阅读表", "阅读角", "阅读台",
  ...
  const LANGUAGE_EXTRA_WORDS = filterNovelLanguageWords(
    EXTENSION_CANDIDATE_POOL,
  ...
  const EXTRA_GROUP_ACTIONS = uniqueList([
    "玩耍", "学习", "读书", "写字", "画画", "唱歌", "跳舞", "跑步", "排队", "吃饭", "喝水"
  ]);
  ...
  const merged = uniqueByEntry([...target, ...extraLanguageEntries]);
  if (target !== merged) {
    target.length = 0;
  ...
  if (typeof BRIDGE_VOCAB_FULL !== "undefined" && Array.isArray(BRIDGE_VOCAB_FULL)) {
    const existingKeys = new Set(BRIDGE_VOCAB_FULL.map((item) => getEntryIdentityKey(item)).filter(Boolean));
    extraLanguageEntries.forEach((item) => {
      const key = getEntryIdentityKey(item);
      if (!key || existingKeys.has(key)) return;
      existingKeys.add(key);
      BRIDGE_VOCAB_FULL.push(item);
    });
  }
})();
