...
```bash
python .Codex/skills/largebase-structured-scan/scan.py extract \
  --scope src docs references \
  --topic refactor-core
```
...
### Step 5：导出摘要到 AGENTS.md（默认执行）

```bash
python .Codex/skills/largebase-structured-scan/scan.py export-to-Codex-md \
  --db docs/scan/YYYY-MM-DD-refactor-core/scan.db \
  --Codex-md AGENTS.md
```

- 将核心模块、关键约束和高风险影响点写入 AGENTS.md 的标记区块
