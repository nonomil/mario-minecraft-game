# codex-workflow-constants.md — 全局硬约束与常量
...
- 默认由 Codex 直接执行需求澄清、实现、验证和交付
- 仅当需要补充能力时调用 Claude Code（MCP Server）
- 由 Claude Code 反向调用 Codex 仅作为兼容路径，不是默认路径

---

## Claude Code 反向调用 Codex 规范（仅兼容场景）

> 默认主流程由 Codex 执行；仅在 Codex 明确委派时，Claude Code 才调用 Codex MCP。
...
- 每次调用必须通过 Prompt 约束 + Hook 拦截 + Codex 验收三层防护

### Codex 调用 Claude Code 前（强制注入 Constraints）

在给 Claude Code 的 Prompt 中必须包含：
...
- Claude Code 如执行了删除类命令，立即中止并人工复核
...
> 以下规范适用于 Codex 主流程与 Claude Code 辅助调用场景。
>
> Claude Code 独立会话下仍可使用其内置 `Skill` 工具（详见 `CLAUDE.md`）。
...
- Codex 主流程下优先使用此规范；Claude Code 独立会话可使用其内置 `Skill` 工具
...
### Skill 选择规则（Codex 主流程）

在 Codex 主流程中，按任务类型选择 skill：
...
并行任务（worktree）额外门禁：
```bash
python .codex/scripts/verify_parallel_scope.py \
...
- 刷新命令：`python .codex/skills/largebase-structured-scan/scan.py export-to-claude-md --db [latest_scan_db] --claude-md CLAUDE.md`
...
```markdown
> 参见 `workflows/codex-workflow-constants.md` 中的"Claude Code 反向调用 Codex 规范（仅兼容场景）"
```
...
| 工具 | 角色 | 擅长 | 何时用 |
|------|------|------|--------|
| Antigravity + Opus 高推理会话 | 需求分析师 + 架构师 | 深度需求澄清、架构决策、发现歧义（1M token 上下文，SWE-bench 74.5%） | 仅用于最初 Plan 生成，质量最高 |
| Codex 高推理规划会话 | 日常规划者 | 单任务分析、文件搜索、改动预览 | 日常开发中每个任务前 |
...
> 由 `codex-workflow-init.md` 流程写入，后续所有工作流读取此处常量，不再询问用户。
> 未运行 init 流程时，使用下方默认值。

| 常量 | 默认值 | 说明 |
|------|--------|------|
| `BRANCH_MODE` | `temporary` | 分支管理模式：`temporary`（临时分支）/ `bare`（Bare Repo）|
| `WORKTREE_BASE` | `.worktree` | worktree 目录前缀，可选：`worktrees` / `.worktree` / `.` |
| `MAIN_BRANCH` | `master` | 主分支名，可选：`master` / `main` |
| `PROJECT_ROOT` | `G:/UserCode/清华学习龙虾OpenMAIC` | 项目根目录绝对路径 |
| `INIT_DATE` | `2026-03-17` | 初始化日期 |
...
- 2026-02-27：移动到 `.codex/workflows/` 目录，更新引用方式
