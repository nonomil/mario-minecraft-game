...
$scan_args = @(
  ".codex/skills/largebase-structured-scan/scan.py",
  "scan",
  "--mode", $scan_mode,
  "--scope"
) + $scope_paths + @(
...
& $python_exec .codex/skills/largebase-structured-scan/scan.py load --load $scan_data_path --db $db_path
if ($LASTEXITCODE -ne 0) {
  throw "load failed; flow stopped."
}

Write-Step "Run query validation"
& $python_exec .codex/skills/largebase-structured-scan/scan.py query --query $query_text --type all --db $db_path
if ($LASTEXITCODE -ne 0) {
