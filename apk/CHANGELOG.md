...
- **词库净化回归覆盖**: 新增 `tests/unit/bridge-vocab-curation.test.mjs`，把汉字例词、幼小衔接语文/数学/英语净化规则固化为回归测试。

### Fixed
- **汉字例词污染**: 修正 800 字汉字包里仍残留的成人化/词典腔例词，统一把 `操` 等条目改回包内可用、儿童可理解的自然组词。
- **幼小衔接词库噪声**: 继续清理语文里的教师任务口吻、数学里的抽象题型词、英语里的伪自然拼读字符串，并确保桥接英语包加载独立英语补充文件。
- **词卡提示冗余**: 汉字模式与幼小衔接模式继续收口词卡元信息/提示文本，避免展示不必要的“层级/说明句”。

### Changed
- **README 主界面说明**: 更新仓库首页说明，补齐当前环境、BOSS、学习模式、词库结构和近期 Git 演进概览。
- **版本发布**: 统一版本元数据到 `v1.19.48 / build 99`，同步缓存戳、APK 构建信息和发布说明。

## [1.19.46] - 2026-03-20
### Added
- **学习线分包更清晰**: 汉字包与拼音包在 manifest 与设置列表中拆成独立入口，显示名称和分组更直观。
- **幼小衔接课堂化收口**: 继续整理幼小衔接/汉字/拼音模式的卡片、挑战与 HUD 文案，使内容更贴近学前到二年级学习场景。

### Fixed
- **TTS 回退链路**: 修复 APK provider 不可用时整套词库都无法发音的问题，恢复到 Web TTS / 备用 provider 的安全回退。
- **抽词重复与串包**: 修复汉字/拼音模式仍按旧 `en` 键去重与排除，导致反复刷出同词、跟随短语和挑战候选不稳定的问题。
- **模式设置误导**: 修复英语/汉字模式仍显示幼小衔接层级选择，以及拼音模式圆圈默认显示拼音、提示过多的问题。

### Changed
- **词卡与测验体验**: 精简幼小衔接单词卡无用提示，统一挑战标题、上下文徽标、纠错文案与展示优先级。
- **版本发布**: 升级版本元数据到 `v1.19.46 / build 98`，同步缓存版本和发布说明。
