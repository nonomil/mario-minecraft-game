# codex-workflow-parallel｜多功能并行开发流程

> 本文档中的所有规则遵循 `codex-workflow-constants.md` 中的全局约束
> 触发条件：任务数 ≥ 2 且可解耦，需要多个 worktree 同时开发
> 入口：从 `CODEX.md` 场景路由跳转至此
> 注意：并行开发通常也是复杂模式，先完成 `codex-workflow-complex.md` Phase 1-4.5 再进入此文档
> **路径常量**：本文档中所有 worktree 路径使用 `[WORKTREE_BASE]` 占位符，实际值读取 `codex-workflow-constants.md` 中的 `WORKTREE_BASE` 常量（默认：`worktrees`）
...
让 Opus/Codex 在拆任务时同步产出，在 prompt 里加：
```text
同时输出文件影响范围表，列格式固定为：
...
```text
# worktree-task-1 的 Codex 执行模板
在对应 worktree 的 Codex 会话中执行：
- Context：技术栈、steps 文档、任务描述、禁止修改文件
- Task：按 steps 文档逐任务执行；每个任务完成后提交
- Constraints：仅限指定文件、不引入新依赖、commit 前 diff ≤ 200 行
- Acceptance：任务完成并提交；测试通过
```

**每个 worktree 内部的 TDD 执行循环遵循 `AI开发-PLan-Program-Debug-Claude和Codex协作/05-Codex+Claude Code+Git Worktree-功能分支开发流程模板.md` Phase 3，包括：**
- 步骤 3.1：Codex 在规划阶段搜索现有代码，回答 Linus 三问
- 步骤 3.4.5：每个任务完成后必须执行多轮 Review（Codex 轮次1 + Claude Code 轮次2），连续 2 轮无新问题才可提交
...
**冲突处理：**
- 出现冲突 → 立即停止自动合并，Codex 先汇报冲突文件与原因（解耦不充分？）
...
- 某个 worktree Claude Code 连续失败 3 次 → Codex 接管该 worktree，手动处理
