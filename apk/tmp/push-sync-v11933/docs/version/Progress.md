...
- 主要完成项：
  - 修复 `启动游戏.bat`、`http://localhost:4173/Game.html`、离线 `android-app/web/index.html` 与 APK 单文件产物中词库切换不一致的问题。
  - `build-singlefile` 现可识别 single-quoted manifest 路径，并内联全部本地 `src/*.js` / `words/vocabs/*.js` 脚本。
  - 为 Minecraft 中级/完整词库和幼儿园汉字词库补齐浏览器兼容导出，切换后不再落回默认词库。
  - 设置保存立即同步账号当前词库；启动器增加 `dev-reset.html` 清理旧 SW / cache。
- 验证结果：
  ...
  - `file:///.../android-app/web/index.html` 连续切换 `vocab.minecraft.intermediate` / `vocab.minecraft.full` / `vocab.kindergarten.hanzi` 均通过
