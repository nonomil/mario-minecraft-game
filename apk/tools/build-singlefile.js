...
function extractVocabFilesFromManifest(manifestJs) {
  const files = [];
  // Match both single-quoted and double-quoted local vocab paths.
  const re = /(["'])(words\/vocabs\/[^"']+\.js)\1/g;
  let m;
  while ((m = re.exec(manifestJs))) {
    files.push(decodeUnicodeEscapes(m[2]));
  }
...
function inlineRemainingLocalScripts(html, projectRoot) {
  const re = /<script\b[^>]*\bsrc=(["'])([^"']+)\1[^>]*>\s*<\/script>/gi;
  let out = html;
  let m;
  const candidates = [];
  ...
  while ((m = re.exec(html))) {
    const tag = m[0];
    const src = normalizeRelPath(m[2]);
    const isLocalScript = src.toLowerCase().startsWith("src/") || src.toLowerCase().startsWith("words/vocabs/");
    if (!isLocalScript) continue;
    candidates.push({ tag, src });
  }

  for (const { tag, src } of candidates) {
    const jsPath = path.join(projectRoot, ...src.split("/"));
    const content = readText(jsPath);
    out = out.replace(tag, makeInlineScript(content));
  }

  return out;
}

function assertNoRemainingLocalScriptSrc(html) {
  const remaining = html.match(/<script\b[^>]*\bsrc=(["'])(src\/|words\/vocabs\/)[^"']+\1/i);
  if (remaining) {
    throw new Error(`build-singlefile: unresolved local script src remains in output HTML: ${remaining[0]}`);
  }
}
...
function buildSingleFile({ projectRoot, templateHtmlPath, outPath }) {
  ...
  html = inlineRemainingLocalScripts(html, projectRoot);
  assertNoRemainingLocalScriptSrc(html);
