...
### Fixed
- **Launcher / Offline / APK Vocabulary Switching**: 修复启动器、本地 localhost 页面、离线单文件与 APK 产物之间的词库切换不一致。
  - `build-singlefile` 现在支持从 single-quoted manifest 提取词库文件，并强制内联所有本地脚本，防止 `file://` / APK 模式残留 `src/*.js` 外链
  - `minecraft_intermediate.js`、`minecraft_words_full.js`、`幼儿园汉字.js` 补齐浏览器兼容导出，确保 manifest 命中的 pack 真能取到词
  - `saveSettings()` 立即写回 `currentAccount.vocabulary.currentPack`，避免切换后再次启动被旧账号值覆盖
  - `启动游戏.bat` 新增 reset 页面，先清理旧 Service Worker / cache，再进入游戏

### Improved
- **Regression Coverage**: 词库切换回归覆盖扩展到离线单文件和启动脚本链路。
  - 新增 `tests/unit/offline-vocab-bundle-regression.test.mjs`
  - 新增 `tests/unit/launch-game-port-cleanup.test.mjs`
  - 新增 `tests/e2e/specs/p0-launcher-sw-reset.spec.mjs`
  - `tests/e2e/specs/p0-vocab-pack-switch.spec.mjs` 现在验证 Minecraft 完整词库的真实数据特征，而不是仅看 `wordCount > 100`
...
- `file:///.../android-app/web/index.html` 离线连续切换验证通过：Minecraft 中级 `60` 词、Minecraft 完整 `1163` 词、幼儿园汉字 `60` 词

### Version Metadata
- `version.json` -> `versionCode/buildNumber 86`, `versionName 1.19.33`
- `package.json` -> `1.19.33`
- `android-app/package.json` -> `1.19.33`
- `android-app/android/app/build.gradle` -> `versionCode 86`, `versionName 1.19.33`
- `android-app/web/build-info.json` -> `version 1.19.33`, `buildNumber 86`, `gitCommit 55f4659`
- `service-worker.js` -> cache `v1.19.33`
- `Game.html` -> cache-busting `v1.19.33`
