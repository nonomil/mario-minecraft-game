# codex-workflow-largebase｜大型代码库与多参考文档流程（结构化数据版）

> 本文档中的所有规则遵循 `codex-workflow-constants.md` 中的全局约束
> 适用：源码文件多、跨模块改动、多份参考文档并存、需要先产出"可消费的数据文档"再规划实施。
> 目标：Codex 生成结构化扫描包，Codex 基于扫描包写 Plan，Claude Code 负责交叉审查，避免盲目搜索。
> 核心：三层结构（JSON 存储 + SQLite 查询 + Markdown 展示），参考 memory-system 设计。
> 推荐：在该场景优先调用技能 `largebase-structured-scan`（见 `.codex/skills/largebase-structured-scan/SKILL.md`）。
> 详细分析：`.codex/skills/largebase-structured-scan/ANALYSIS.md`
...
4. 确认"只扫描不改码"约束。
5. 执行初始化脚本（优先）：
   - `pwsh .codex/scripts/run_largebase_scan.ps1 -scan_mode [M1|M2|M3|M4] -scan_topic [topic] -scope_paths [paths...] -reference_docs [docs...]`
...
### Phase E：影响矩阵与执行摘要（Codex + Claude Code）

1. 生成 `05-impact-matrix.md`
2. 生成 `06-exec-brief.md`
3. Codex 根据执行摘要路由：
   - 建议任务数 `=1` → `codex-workflow-complex.md`（跳过其 Phase 0）
   - 建议任务数 `>=2` → `codex-workflow-parallel.md`
   - 扫描结论以缺陷修复为主 → `codex-workflow-debug.md`
4. 若当前使用低成本 API 扫描，Codex 必须在路由前提示：
   ```
   扫描已完成，所有产物已生成。
   如果你之前切换了低成本 API，请现在切换回高质量 API（如 yunyi profile），然后重启 Codex 继续审查和开发。
   ```
...
若任何一项失败：
- 禁止进入开发阶段
- 通过 `codex_reply` 要求补齐，不允许 Codex 自行脑补
...
在大型代码库场景，建议优先触发 `.codex/skills/largebase-structured-scan/SKILL.md`。
