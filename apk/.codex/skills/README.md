...
### 6. `web-ui-screenshot-audit/`
**用途**：Web 前端截图审查与报告技能

**何时使用**：需要对本地 `localhost` 页面做截图取证、流程验收、界面评审，并输出本地 Markdown 报告时调用

**功能**：
- 规划桌面端 / 移动端 / 流程关键状态截图
- 要求落地 `capture_manifest.json`
- 汇总测试结果、构建结果与截图证据
- 输出带优先级建议的 UI 审查报告

**调用方式**：
```bash
node C:/Users/Administrator/.codex/superpowers/.codex/superpowers-codex use-skill web-ui-screenshot-audit
```
...
- 不要直接执行 `~/.codex/superpowers/.codex/superpowers-codex bootstrap`
- 必须通过 `node C:/Users/Administrator/.codex/superpowers/.codex/superpowers-codex bootstrap` 调起脚本
- 原因是 `superpowers-codex` 在当前环境中是无扩展名 Node 脚本，直接执行会报文件关联错误
