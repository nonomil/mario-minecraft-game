...
### 新功能
- 学习进度结构化：`progress.vocab[packId].unique[word]` 从旧的数值标记升级为对象结构，记录 `seen/correct/wrong/lastSeen/quality`（L-8）
- 跨局质量继承：词库激活时回灌历史 `quality` 到 `wordPicker`，保持错词强化与熟词降频策略连续（L-8a）
- 游戏结算新增”本局接触词汇”区域，支持点击词条朗读复习（L-9）

### 兼容
- 旧存档 `unique[word] = 1` 自动迁移为新结构，兼容读取无需手动处理
...
- 修复多弹窗叠加时游戏意外恢复的问题（G-2）
  ...
  - 统一弹窗暂停/恢复入口为 `pushPause` / `popPause`
- 修复花朵装饰悬空问题
  - 花朵生成点改为贴地（底部对齐地面）
  - 花朵渲染增加茎与叶片，视觉上改为“地面生长”
...
### 修复
- **树木悬浮问题**：修复视口缩放后树木悬浮在空中的渲染错误
  - `drawPixelTree` 函数使用硬编码像素值（树宽80、树高140等），在视口缩放后无法与实体位置对齐
  - 引入缩放因子 `s = blockSize / 50`，将所有硬编码尺寸改为相对 blockSize 的动态值
  - 修复所有树木类型（oak、birch、spruce、pine、palm、cactus 等）的渲染尺寸
  - 修复文件：`apk/src/modules/14-renderer-main.js`

### 技术改进
- 树木视觉大小现在与实体碰撞盒大小保持一致
- 树木底部正确对齐地面，无论视口大小如何变化
...
### 修复
- **横屏全屏错位问题深度修复**：修复 Android 横屏场景下视口变化后地面、角色与世界对象错位的问题
  - `groundY` 改为锚定画布底部物品栏上沿，避免仅按 `scale.y` 缩放导致地面偏移
  - 新增视口变化后的世界重定位逻辑（角色、平台、物品、敌人、投射物等统一按新画布比例重映射）
  - 修复文件：`apk/src/main.js`、`src/main.js`

### 测试
- 新增移动端回归测试，模拟横屏 + 安全区变化，校验地面与角色保持在可视区域内
  - 测试文件：`tests/requirements.spec.js`
